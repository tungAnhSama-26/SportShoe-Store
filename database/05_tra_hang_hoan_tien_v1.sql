...
BEGIN
    CREATE TABLE dbo.lich_su_phieu_tra_hang (
        ...
        CONSTRAINT fk_lspth_phieu FOREIGN KEY (phieu_tra_hang_id) REFERENCES dbo.phieu_tra_hang(id) ON DELETE CASCADE,
        CONSTRAINT fk_lspth_nhan_vien FOREIGN KEY (nhan_vien_id) REFERENCES dbo.nhan_vien(id)
    );
    CREATE INDEX ix_lspth_phieu ON dbo.lich_su_phieu_tra_hang(phieu_tra_hang_id, ngay_tao DESC);
END;
...
BEGIN
    CREATE TABLE dbo.hinh_anh_tra_hang (
        ...
        CONSTRAINT ck_hath_loai_anh CHECK (loai_anh IN (1,2)),
        CONSTRAINT fk_hath_phieu FOREIGN KEY (phieu_tra_hang_id) REFERENCES dbo.phieu_tra_hang(id) ON DELETE CASCADE,
        CONSTRAINT fk_hath_chi_tiet FOREIGN KEY (phieu_tra_hang_chi_tiet_id) REFERENCES dbo.phieu_tra_hang_chi_tiet(id)
    );
    CREATE INDEX ix_hath_phieu ON dbo.hinh_anh_tra_hang(phieu_tra_hang_id);
END;
...
IF NOT EXISTS (SELECT 1 FROM sys.foreign_keys WHERE name = 'fk_tt_phieu_tra_hang')
    ALTER TABLE dbo.thanh_toan
        ADD CONSTRAINT fk_tt_phieu_tra_hang
        FOREIGN KEY (phieu_tra_hang_id) REFERENCES dbo.phieu_tra_hang(id);

IF NOT EXISTS (SELECT 1 FROM sys.foreign_keys WHERE name = 'fk_tt_giao_dich_goc')
    ALTER TABLE dbo.thanh_toan
        ADD CONSTRAINT fk_tt_giao_dich_goc
        FOREIGN KEY (giao_dich_goc_id) REFERENCES dbo.thanh_toan(id);

IF NOT EXISTS (
    SELECT 1 FROM sys.indexes
    WHERE object_id = OBJECT_ID('dbo.thanh_toan') AND name = 'ix_tt_phieu_tra_hang'
)
    CREATE INDEX ix_tt_phieu_tra_hang ON dbo.thanh_toan(phieu_tra_hang_id);
...
SELECT N'phieu_tra_hang' AS bang, COUNT(*) AS so_ban_ghi FROM dbo.phieu_tra_hang
UNION ALL
SELECT N'phieu_tra_hang_chi_tiet', COUNT(*) FROM dbo.phieu_tra_hang_chi_tiet
UNION ALL
SELECT N'lich_su_phieu_tra_hang', COUNT(*) FROM dbo.lich_su_phieu_tra_hang
UNION ALL
SELECT N'hinh_anh_tra_hang', COUNT(*) FROM dbo.hinh_anh_tra_hang;
GO
