...
<template>
  <header
    :class="[
      ...
      dangCuon
        ? 'border-red-200/70 bg-white/95 shadow-[0_10px_30px_rgba(127,29,29,0.12)] backdrop-blur'
  ...
  >
    <div class="mx-auto flex max-w-7xl items-center justify-between px-6 py-3 lg:px-10">
      ...
      <div class="flex items-center md:hidden">
        <button class="relative text-slate-900 transition hover:text-red-600" aria-label="Giỏ hàng">
          ...
          <span class="absolute -right-2 -top-2 flex h-4 min-w-4 items-center justify-center rounded-full bg-red-600 px-1 text-[10px] font-bold text-white">
            2
          </span>
        </button>
      </div>
    ...
    >
      <nav class="flex flex-col gap-6 p-6 text-base font-semibold text-slate-800">
        <router-link :to="{ path: '/' }" @click="toggleMenu" class="transition hover:text-red-600">Trang chủ</router-link>
        <router-link :to="{ path: '/', hash: '#bo-suu-tap' }" @click="toggleMenu" class="transition hover:text-red-600">Danh mục</router-link>
        <router-link :to="{ path: '/', hash: '#noi-bat' }" @click="toggleMenu" class="transition hover:text-red-600">Nổi bật</router-link>
        <router-link :to="{ path: '/', hash: '#gia-tri' }" @click="toggleMenu" class="transition hover:text-red-600">Giới thiệu</router-link>
      </nav>
      <div class="mb-8 flex items-center gap-6 border-t border-slate-100 px-6 pt-6">
        <button class="flex items-center gap-2 text-slate-900 transition hover:text-red-600" aria-label="Tìm kiếm">
          <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <circle cx="11" cy="11" r="7" />
            <path d="m20 20-3.5-3.5" />
          </svg>
        ...
        <button class="flex items-center gap-2 text-slate-900 transition hover:text-red-600" aria-label="Tài khoản">
          <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <path d="M20 21a8 8 0 1 0-16 0" />
            <circle cx="12" cy="7" r="4" />
          </svg>
