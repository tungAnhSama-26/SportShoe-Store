...
async function loadData(page = 0) {
  loading.value = true
  try {
    const res = await congNgheDemApi.list(keyword.value || undefined, page)
...
async function handleDelete(item) {
  if (!confirm(`Xóa công nghệ đệm "${item.ten}"?`)) return
  try { await congNgheDemApi.delete(item.id); showToast('Xóa thành công'); loadData(currentPage.value) }
  catch (e) { showToast(e.message || 'Lỗi xóa', 'error') }
}

async function handleToggleStatus(item) {
  try { await congNgheDemApi.toggleStatus(item.id, item.trangThai === 1 ? 0 : 1); showToast('Cập nhật trạng thái thành công'); loadData(currentPage.value) }
...
<template>
  <div class="p-6">
    ...
    <div class="mb-6 flex justify-between items-center">
      <h1 class="text-2xl font-bold text-gray-800">Quản Lý Công Nghệ Đệm</h1>
      ...
      <table class="w-full text-sm">
        <thead class="bg-gray-50 border-b border-gray-100">
          <tr>
            ...
            <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Tên công nghệ</th>
