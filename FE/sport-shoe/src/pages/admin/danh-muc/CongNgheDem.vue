...
function generateCode() {
  return generateAttributeCode(form.ten, 'CND', 'CNDEM', codeSeed.value)
}
...
function validate() {
  ...
  if (!form.ten || !form.ten.trim()) errors.ten = 'Vui lòng nhập tên công nghệ đệm'
    ...
    errors.ten = `Tên công nghệ đệm phải từ 4 đến ${TEN_MAX_LENGTH} ký tự`
  }
...
async function handleSave() {
  ...
  try {
    ...
    if (modalMode.value === 'add') {
      await congNgheDemApi.create(body)
      showSuccess('Tạo thành công')
      showModal.value = false
      keyword.value = ''
      loadData(0)
    } else {
      await congNgheDemApi.update(selectedItem.value.id, body)
          ...
          <div class="bg-white rounded-md shadow-2xl w-full max-w-md">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <h2 class="text-lg font-semibold text-gray-800">
                {{ modalMode === 'add' ? 'Thêm công nghệ đệm' : modalMode === 'edit' ? 'Cập nhật công nghệ đệm' : 'Chi tiết công nghệ đệm' }}
              </h2>
            ...
            <div class="p-6 space-y-4">
              ...
              <div>
                <label class="block text-xs font-medium text-gray-700 mb-1">Tên công nghệ đệm <span class="text-rose-500">*</span></label>
                <input
                  v-model="form.ten"
