...
async function loadData(page = 0) {
  loading.value = true
  try {
    const res = await kichCoApi.list(keyword.value || undefined, page)
...
function openView(item) { clearForm(); Object.assign(form, { giaTri: item.giaTri, ghiChu: item.ghiChu || '' }); selectedItem.value = item; modalMode.value = 'view'; showModal.value = true }
...
<template>
  <div class="p-6">
    ...
    <div class="mb-6 flex justify-between items-center">
      <h1 class="text-2xl font-bold text-gray-800">Quản Lý Kích Cỡ</h1>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 mb-4">
      <div class="flex gap-3">
        <div class="relative flex-1">
          ...
          <input v-model="keyword" @keyup.enter="doSearch" placeholder="Tìm kiếm kích cỡ..." class="w-full pl-9 pr-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-rose-400" />
        </div>
      ...
      <table class="w-full text-sm">
        <thead class="bg-gray-50 border-b border-gray-100">
          <tr>
            ...
            <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500">Kích cỡ</th>
            <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500">Ghi chú</th>
        ...
        <tbody class="divide-y divide-gray-50">
          ...
          <tr v-for="(item, idx) in items" :key="item.id" v-else class="hover:bg-gray-50">
            ...
            <td class="px-4 py-3">
              <span class="inline-flex items-center justify-center w-12 h-8 bg-rose-50 text-rose-700 font-semibold text-sm rounded-lg border border-rose-100">{{ item.giaTri }}</span>
            </td>
            <td class="px-4 py-3 text-gray-500 text-xs">{{ item.ghiChu || '—' }}</td>
