...
async function loadData(page = 0) {
  loading.value = true
  try {
    const res = await loaiGiayApi.list(keyword.value || undefined, page)
...
<template>
  <div class="p-6">
    ...
    <div class="mb-6 flex justify-between items-center">
      <h1 class="text-2xl font-bold text-gray-800">Quản Lý Loại Giày</h1>
      ...
      <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-gray-50">
          ...
          <tr v-for="(item, idx) in items" :key="item.id" v-else class="hover:bg-gray-50">
            ...
            <td class="px-4 py-3 font-mono text-xs font-semibold text-rose-700 bg-rose-50 rounded">{{ item.ma }}</td>
