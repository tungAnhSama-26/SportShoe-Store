...
async function loadData(page = 0) {
  ...
  try {
    ...
    showError(getDisplayErrorMessage(e, 'Không thể tải danh sách loại giày'))
  } finally {
    loading.value = false
  }
}
...
function generateCode() {
  return generateAttributeCode(form.ten, 'LG', 'LOAI', codeSeed.value)
}
...
function validate() {
  ...
  if (!form.ten || !form.ten.trim()) errors.ten = 'Vui lòng nhập tên loại giày'
  else if (form.ten !== form.ten.trim()) {
    errors.ten = 'Tên loại giày không được chứa khoảng trắng ở đầu hoặc cuối'
  }
  else if (ten.length < 4 || ten.length > TEN_MAX_LENGTH) {
    errors.ten = `Tên loại giày phải từ 4 đến ${TEN_MAX_LENGTH} ký tự`
  }
...
async function handleSave() {
  ...
  if (modalMode.value === 'add') {
    const isConfirmed = await showConfirm('Xác nhận thêm mới loại giày này?')
    if (!isConfirmed) return
  } else {
    const isConfirmed = await showConfirm('Xác nhận lưu thay đổi loại giày này?')
    if (!isConfirmed) return
  }
  ...
  try {
    ...
    if (modalMode.value === 'add') {
      await loaiGiayApi.create(body)
      showSuccess('Tạo thành công')
      showModal.value = false
      keyword.value = ''
      loadData(0)
    } else {
      await loaiGiayApi.update(selectedItem.value.id, body)
    ...
    showError(getDisplayErrorMessage(e, 'Không thể lưu loại giày'))
  } finally {
    saving.value = false
  }
}
...
async function handleToggleStatus(item) {
  ...
  const isConfirmed = await showConfirm(`Xác nhận ${actionLabel} nhanh loại giày "${item.ten}"?`)
  if (!isConfirmed) return

  updatingStatusId.value = item.id
  try {
    await loaiGiayApi.toggleStatus(item.id, nextTrangThai)
    showSuccess('Cập nhật trạng thái thành công')
    loadData(currentPage.value)
  } catch (e) {
    showError(getDisplayErrorMessage(e, 'Không thể cập nhật trạng thái loại giày'))
  } finally {
...
async function xuatExcel() {
  ...
  try {
    const res = await loaiGiayApi.list(keyword.value || undefined, 0, Math.max(totalItems.value, pageSize.value))
    const exported = exportRowsToExcel({
      filename: 'quan-ly-loai-giay',
      sheetName: 'LoaiGiay',
      columns: [
        { label: 'STT', value: (_, index) => index + 1 },
        { label: 'Mã', key: 'ma' },
        { label: 'Tên loại giày', key: 'ten' },
    ...
    showError(getDisplayErrorMessage(e, 'Không thể xuất Excel loại giày'))
  }
}
</script>
          ...
          <div class="bg-white rounded-md shadow-2xl w-full max-w-md">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <h2 class="text-lg font-semibold text-gray-800">
                {{ modalMode === 'add' ? 'Thêm loại giày' : modalMode === 'edit' ? 'Cập nhật loại giày' : 'Chi tiết loại giày' }}
              </h2>
            ...
            <div class="p-6 space-y-4">
              ...
              <div>
                <label class="block text-xs font-medium text-gray-700 mb-1">Tên loại giày <span class="text-rose-500">*</span></label>
                <input
                  v-model="form.ten"
