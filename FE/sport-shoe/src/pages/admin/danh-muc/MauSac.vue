...
async function handleSave() {
  ...
  try {
    ...
    if (modalMode.value === 'add') {
      await mauSacApi.create(body)
      showSuccess('Tạo thành công')
      showModal.value = false
      keyword.value = ''
      loadData(0)
    } else {
      await mauSacApi.update(selectedItem.value.id, body)
          ...
          <div class="bg-white rounded-md shadow-2xl w-full max-w-md">
            ...
            <div class="p-6 space-y-4">
              <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label class="block text-xs font-medium text-gray-700 mb-1">Tên màu <span class="text-rose-500">*</span></label>
                  <input
                    v-model="form.ten"
                    @input="onTenInput"
