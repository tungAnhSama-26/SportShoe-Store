...
async function loadData(page = 0) {
  ...
  try {
    ...
    showError(getDisplayErrorMessage(e, 'Không thể tải danh sách đế giày'))
  } finally {
    loading.value = false
  }
}
...
function generateCode() {
  return generateAttributeCode(form.ten, 'DG', 'DEGIAY', codeSeed.value)
}
...
function validate() {
  ...
  if (!form.ma.trim()) errors.ma = 'Không thể tự tạo mã đế giày'
  if (!form.ten || !form.ten.trim()) errors.ten = 'Vui lòng nhập tên đế giày'
  else if (form.ten !== form.ten.trim()) {
    errors.ten = 'Tên đế giày không được chứa khoảng trắng ở đầu hoặc cuối'
  }
  else if (ten.length < 4 || ten.length > TEN_MAX_LENGTH) {
    errors.ten = `Tên đế giày phải từ 4 đến ${TEN_MAX_LENGTH} ký tự`
  }
...
async function handleSave() {
  ...
  if (modalMode.value === 'add') {
    const isConfirmed = await showConfirm('Xác nhận thêm mới đế giày này?')
    if (!isConfirmed) return
  } else {
    const isConfirmed = await showConfirm('Xác nhận lưu thay đổi đế giày này?')
    if (!isConfirmed) return
  }
...
async function handleToggleStatus(item) {
  ...
  const isConfirmed = await showConfirm(`Xác nhận ${actionLabel} nhanh đế giày "${item.ten}"?`)
  if (!isConfirmed) return

  updatingStatusId.value = item.id
  try {
    await deGiayApi.toggleStatus(item.id, nextTrangThai)
    showSuccess('Cập nhật trạng thái thành công')
    loadData(currentPage.value)
  } catch (e) {
    showError(getDisplayErrorMessage(e, 'Không thể cập nhật trạng thái đế giày'))
  } finally {
...
async function xuatExcel() {
  ...
  try {
    const res = await deGiayApi.list(keyword.value || undefined, 0, Math.max(totalItems.value, pageSize.value))
    const exported = exportRowsToExcel({
      filename: 'quan-ly-de-giay',
      sheetName: 'DeGiay',
      columns: [
        { label: 'STT', value: (_, index) => index + 1 },
        { label: 'Mã', key: 'ma' },
        { label: 'Tên đế giày', key: 'ten' },
    ...
    showError(getDisplayErrorMessage(e, 'Không thể xuất Excel đế giày'))
  }
}
</script>
...
<template>
  <DanhMucPageShell
    title="Quản lý đế giày"
    add-label="Thêm đế giày"
    list-title="Danh sách đế giày"
    search-placeholder="Tìm theo mã, tên đế giày..."
    :keyword="keyword"
    :page-size="pageSize"
          ...
          <div class="bg-white rounded-md shadow-2xl w-full max-w-md">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <h2 class="text-lg font-semibold text-gray-800">
                {{ modalMode === 'add' ? 'Thêm đế giày' : modalMode === 'edit' ? 'Cập nhật đế giày' : 'Chi tiết đế giày' }}
              </h2>
