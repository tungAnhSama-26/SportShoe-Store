...
async function loadData(page = 0) {
  loading.value = true
  try {
    const res = await thuongHieuApi.list(keyword.value || undefined, page)
...
function openView(item) { clearForm(); Object.assign(form, { ma: item.ma, ten: item.ten, xuatXu: item.xuatXu || '', logoUrl: item.logoUrl || '', website: item.website || '', moTa: item.moTa || '' }); selectedItem.value = item; modalMode.value = 'view'; showModal.value = true }
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 mb-4">
      <div class="flex gap-3">
        <div class="relative flex-1">
          ...
          <input v-model="keyword" @keyup.enter="doSearch" placeholder="Tìm kiếm..." class="w-full pl-9 pr-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-rose-400" />
        </div>
      ...
      <table class="w-full text-sm">
        <thead class="bg-gray-50 border-b border-gray-100">
          <tr>
            ...
            <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 w-16">Logo</th>
            <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500">Mã</th>
            <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500">Tên thương hiệu</th>
            <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500">Xuất xứ</th>
        ...
        <tbody class="divide-y divide-gray-50">
          ...
          <tr v-for="(item, idx) in items" :key="item.id" v-else class="hover:bg-gray-50">
            ...
            <td class="px-4 py-3">
              ...
              <div v-else class="h-10 w-10 rounded-lg bg-gray-100 flex items-center justify-center text-xs text-gray-400">N/A</div>
            </td>
            ...
            <td class="px-4 py-3 text-gray-500">{{ item.xuatXu || '—' }}</td>
