...
function validate() {
  ...
  if (!form.ten || !form.ten.trim()) errors.ten = 'Vui lòng nhập tên thương hiệu'
...
async function handleSave() {
  ...
  try {
    ...
    if (modalMode.value === 'add') {
      await thuongHieuApi.create(body)
      showSuccess('Tạo thành công')
      showModal.value = false
      keyword.value = ''
      loadData(0)
    } else {
      await thuongHieuApi.update(selectedItem.value.id, body)
          ...
          <div class="bg-white rounded-md shadow-2xl w-full max-w-md max-h-[90vh] flex flex-col">
            ...
            <div class="overflow-y-auto flex-1 p-6 space-y-4">
              <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label class="block text-xs font-medium text-gray-700 mb-1">Tên thương hiệu <span class="text-rose-500">*</span></label>
                  <input
                    v-model="form.ten"
