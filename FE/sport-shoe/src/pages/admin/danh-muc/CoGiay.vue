...
async function loadData(page = 0) {
  ...
  } catch (e) { showToast(getDisplayErrorMessage(e, 'Không thể tải danh sách cổ giày'), 'error') }
...
function validate() {
  ...
  if (!form.ma.trim()) errors.ma = 'Vui lòng nhập mã cổ giày'
  if (!form.ten.trim()) errors.ten = 'Vui lòng nhập tên cổ giày'
...
async function handleSave() {
  ...
  try {
    ...
    showToast(getDisplayErrorMessage(e, 'Không thể lưu cổ giày'), 'error')
  }
...
async function handleToggleStatus(item) {
  ...
  if (!confirm('Xác nhận ' + actionLabel + ' nhanh cổ giày "' + item.ten + '"?')) return

  updatingStatusId.value = item.id
  try {
    await coGiayApi.toggleStatus(item.id, nextTrangThai); showToast('Cập nhật trạng thái thành công'); loadData(currentPage.value)
  }
  catch (e) { showToast(getDisplayErrorMessage(e, 'Không thể cập nhật trạng thái cổ giày'), 'error') }
...
async function xuatExcel() {
  ...
  try {
    ...
    showToast(getDisplayErrorMessage(e, 'Không thể xuất Excel cổ giày'), 'error')
  }
}
</script>
