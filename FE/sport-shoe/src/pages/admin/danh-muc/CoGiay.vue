...
import { coGiayApi } from '../../../services/danh-muc-api'
...
async function loadData(page = 0) {
  loading.value = true
  try {
    const res = await coGiayApi.list(keyword.value || undefined, page)
...
function openView(item) { clearForm(); Object.assign(form, { ma: item.ma, ten: item.ten, moTa: item.moTa || '' }); selectedItem.value = item; modalMode.value = 'view'; showModal.value = true }
...
async function handleSave() {
  ...
  try {
    ...
    if (modalMode.value === 'add') await coGiayApi.create(body)
    else await coGiayApi.update(selectedItem.value.id, body)
...
async function handleDelete(item) {
  if (!confirm(`Xóa cổ giày "${item.ten}"?`)) return
  try { await coGiayApi.delete(item.id); showToast('Xóa thành công'); loadData(currentPage.value) }
  catch (e) { showToast(e.message || 'Lỗi xóa', 'error') }
}

async function handleToggleStatus(item) {
  try { await coGiayApi.toggleStatus(item.id, item.trangThai === 1 ? 0 : 1); showToast('Cập nhật trạng thái thành công'); loadData(currentPage.value) }
...
<template>
  <div class="p-6">
    ...
    <div class="mb-6 flex justify-between items-center">
      <h1 class="text-2xl font-bold text-gray-800">Quản Lý Cổ Giày</h1>
      <button @click="openAdd" class="flex items-center gap-2 bg-rose-500 hover:bg-rose-600 text-white px-4 py-2 rounded-lg text-sm font-medium"><Plus :size="16" /> Thêm mới</button>
    </div>

    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 mb-4">
      <div class="flex gap-3">
        <div class="relative flex-1"><Search class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" :size="16" /><input v-model="keyword" @keyup.enter="doSearch" placeholder="Tìm kiếm..." class="w-full pl-9 pr-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-rose-400" /></div>
      ...
      <table class="w-full text-sm">
        <thead class="bg-gray-50 border-b border-gray-100">
          <tr>
            ...
            <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Tên cổ giày</th>
        ...
        <tbody class="divide-y divide-gray-50">
          ...
          <tr v-for="(item, idx) in items" :key="item.id" v-else class="hover:bg-gray-50">
            ...
            <td class="px-4 py-3 text-gray-500 text-xs truncate max-w-[200px]">{{ item.moTa || '—' }}</td>
            <td class="px-4 py-3 text-center"><button @click="handleToggleStatus(item)" class="px-2 py-1 rounded-full text-xs font-medium" :class="item.trangThai === 1 ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-600'">{{ item.trangThai === 1 ? 'Hoạt động' : 'Dừng' }}</button></td>
            <td class="px-4 py-3"><div class="flex items-center gap-1 justify-end"><button @click="openView(item)" class="p-1.5 rounded-lg text-gray-500 hover:bg-gray-100 border border-gray-200"><Eye :size="14" /></button><button @click="openEdit(item)" class="p-1.5 rounded-lg text-rose-600 hover:bg-rose-50 border border-rose-200"><Pencil :size="14" /></button><button @click="handleDelete(item)" class="p-1.5 rounded-lg text-red-600 hover:bg-red-50 border border-red-200"><Trash2 :size="14" /></button></div></td>
          </tr>
        </tbody>
      </table>
      ...
      <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" @click.self="showModal = false">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md">
          <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
            <h2 class="text-lg font-semibold text-gray-800">{{ modalMode === 'add' ? 'Thêm cổ giày' : modalMode === 'edit' ? 'Cập nhật cổ giày' : 'Chi tiết cổ giày' }}</h2>
