...
async function loadData(page = 0) {
  loading.value = true
  try {
    const res = await coGiayApi.list(keyword.value || undefined, page, pageSize.value)
    items.value = res.items
    totalItems.value = res.totalItems
    totalPages.value = res.totalPages
    currentPage.value = res.page
  } catch (e) {
    showError(getDisplayErrorMessage(e, 'Không thể tải danh sách cổ giày'))
  } finally {
    loading.value = false
  }
}
...
function generateCode() {
  return generateAttributeCode(form.ten, 'CG', 'COGIAY', codeSeed.value)
}
...
function validate() {
  ...
  if (!form.ten || !form.ten.trim()) errors.ten = 'Vui lòng nhập tên cổ giày'
    ...
    errors.ten = `Tên cổ giày phải từ 4 đến ${TEN_MAX_LENGTH} ký tự`
  }
...
async function handleSave() {
  ...
  if (modalMode.value === 'add') {
    const isConfirmed = await showConfirm('Xác nhận thêm mới cổ giày này?')
    if (!isConfirmed) return
  } else {
    const isConfirmed = await showConfirm('Xác nhận lưu thay đổi cổ giày này?')
    if (!isConfirmed) return
  }
  ...
  try {
    ...
    if (modalMode.value === 'add') {
      await coGiayApi.create(body)
      showSuccess('Tạo thành công')
      showModal.value = false
      keyword.value = ''
      loadData(0)
    } else {
      await coGiayApi.update(selectedItem.value.id, body)
    ...
    showError(getDisplayErrorMessage(e, 'Không thể lưu cổ giày'))
  } finally {
    saving.value = false
  }
}
...
async function handleToggleStatus(item) {
  ...
  const isConfirmed = await showConfirm(`Xác nhận ${actionLabel} nhanh cổ giày "${item.ten}"?`)
  if (!isConfirmed) return

  updatingStatusId.value = item.id
  try {
    await coGiayApi.toggleStatus(item.id, nextTrangThai)
    showSuccess('Cập nhật trạng thái thành công')
    loadData(currentPage.value)
  } catch (e) {
    showError(getDisplayErrorMessage(e, 'Không thể cập nhật trạng thái cổ giày'))
  } finally {
...
async function xuatExcel() {
  ...
  try {
    const res = await coGiayApi.list(keyword.value || undefined, 0, Math.max(totalItems.value, pageSize.value))
    const exported = exportRowsToExcel({
      filename: 'quan-ly-co-giay',
      sheetName: 'CoGiay',
      columns: [
        { label: 'STT', value: (_, index) => index + 1 },
        { label: 'Mã', key: 'ma' },
        { label: 'Tên cổ giày', key: 'ten' },
    ...
    showError(getDisplayErrorMessage(e, 'Không thể xuất Excel cổ giày'))
  }
}
</script>
...
<template>
  <DanhMucPageShell
    title="Quản lý cổ giày"
    add-label="Thêm cổ giày"
    list-title="Danh sách cổ giày"
    search-placeholder="Tìm theo mã, tên cổ giày..."
    :keyword="keyword"
    :page-size="pageSize"
      ...
      <table class="danh-muc-table table-fixed">
        ...
        <thead class="bg-gray-50 border-b border-gray-100">
          <tr>
            ...
            <th class="px-4 py-3 text-left text-xs font-bold text-slate-950 uppercase">Tên cổ giày</th>
          ...
          <div class="bg-white rounded-md shadow-2xl w-full max-w-md">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <h2 class="text-lg font-semibold text-gray-800">
                {{ modalMode === 'add' ? 'Thêm cổ giày' : modalMode === 'edit' ? 'Cập nhật cổ giày' : 'Chi tiết cổ giày' }}
              </h2>
            ...
            <div class="p-6 space-y-4">
              ...
              <div>
                <label class="block text-xs font-medium text-gray-700 mb-1">Tên cổ giày <span class="text-rose-500">*</span></label>
                <input
                  v-model="form.ten"
