...
async function loadData(page = 0) {
  loading.value = true
  try {
    const res = await trongLuongApi.list(keyword.value || undefined, page)
...
function openView(item) { clearForm(); Object.assign(form, { ma: item.ma, giaTri: item.giaTri, moTa: item.moTa || '' }); selectedItem.value = item; modalMode.value = 'view'; showModal.value = true }
...
<template>
  <div class="p-6">
    ...
    <div class="mb-6 flex justify-between items-center">
      <h1 class="text-2xl font-bold text-gray-800">Quản Lý Trọng Lượng</h1>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 mb-4">
      <div class="flex gap-3">
        <div class="relative flex-1">
          ...
          <input v-model="keyword" @keyup.enter="doSearch" placeholder="Tìm kiếm theo mã..." class="w-full pl-9 pr-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-rose-400" />
        </div>
      ...
      <table class="w-full text-sm">
        <thead class="bg-gray-50 border-b border-gray-100">
          <tr>
            ...
            <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Trọng lượng (g)</th>
        ...
        <tbody class="divide-y divide-gray-50">
          ...
          <tr v-for="(item, idx) in items" :key="item.id" v-else class="hover:bg-gray-50">
            ...
            <td class="px-4 py-3 font-medium text-gray-800">{{ item.giaTri }}<span class="text-gray-400 ml-1 text-xs">g</span></td>
        ...
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md">
          ...
          <div class="p-6 space-y-4">
            <div class="grid grid-cols-2 gap-4">
              ...
              <div>
                <label class="block text-xs font-medium text-gray-700 mb-1">Trọng lượng (g) *</label>
