...
<template>
  <div class="schedule-page space-y-5">
      ...
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-sm text-slate-400">
            Tuần: {{ tieuDeNgay }} &nbsp;·&nbsp; Tối đa {{ MAX_PER_SHIFT }} nhân viên / ca
          </p>
        </div>
      </div>
    ...
    <div v-else class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-7">
      <div
        v-for="(ngay, dayIdx) in toanBoNgay"
        :key="dayIdx"
        class="rounded-3xl border bg-white p-3 shadow-sm transition"
        ...
        <div class="space-y-2">
          <div v-for="ca in CA_LIST" :key="ca.key" class="rounded-2xl border p-2" :class="ca.color">
            ...
            <div class="mb-1.5 flex items-center justify-between gap-1">
              <div class="min-w-0">
                <p class="text-[11px] font-bold leading-tight">{{ ca.label }}</p>
                <p class="text-[10px] opacity-60">{{ ca.time }}</p>
              </div>
              <!-- Badge số lượng -->
              <span
                class="shrink-0 rounded-full px-1.5 py-0.5 text-[10px] font-bold"
                :class="soNhanVienTrongCa(ngay, ca.key) >= MAX_PER_SHIFT ? 'bg-current/20 opacity-90' : 'bg-current/10 opacity-70'"
              >
            ...
            <div class="space-y-1">
              <div
                v-for="nv in nhanVienTrongCa(ngay, ca.key)"
                :key="nv.id"
              ...
              >
                ...
                <button
                  type="button"
                  :disabled="dangXoa === recordCuaNVTrongCa(nv.id, ngay, ca.key)?.id"
                  ...
                  :title="`Xóa ${nv.hoTen} khỏi ca`"
                  @click.stop="xoaKhoiCa(nv, ngay, ca.key)"
                >
                  <X class="h-3 w-3" />
                </button>
              </div>
              ...
              <!-- Empty state -->
              <div
                v-if="nhanVienTrongCa(ngay, ca.key).length === 0"
            ...
            <!-- Nút thêm NV -->
            <button
              v-if="!employeeId && soNhanVienTrongCa(ngay, ca.key) < MAX_PER_SHIFT"
              type="button"
          ...
          <div class="w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl">
            ...
            <div class="mb-5 flex items-center justify-between">
              <div class="flex items-center gap-3">
                ...
                <div>
                  <h3 class="font-bold text-slate-800">Thêm nhân viên vào ca</h3>
                  <p class="text-sm text-slate-400">
            ...
            <div class="mb-4 flex items-center gap-3 rounded-2xl bg-slate-50 px-4 py-3 text-sm text-slate-600">
              ...
              <span class="ml-auto shrink-0 text-xs font-semibold text-slate-500">
                {{ modal.ngay ? soNhanVienTrongCa(modal.ngay, modal.ca) : 0 }}/{{ MAX_PER_SHIFT }} nhân viên
              </span>
            </div>
            ...
            <div class="mb-5">
              ...
              <div
                v-if="nhanVienCoTheChon.length === 0"
            ...
            <div class="flex justify-end gap-3">
              ...
              <Button
                variant="primary"
                :loading="modal.dang"
                :disabled="!modal.nhanVienId || nhanVienCoTheChon.length === 0"
                @click="xacNhanThem"
              >
                <template #prefix>
