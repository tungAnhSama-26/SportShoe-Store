...
function createUsernameSegment(value) {
...
function taoTenDangNhapKhachHang() {
...
watch(
  () => form.value.hoTen,
  (v) => {
    if (laMoi) formDiaChi.value.hoTen = v;
  },
);
watch(
  () => form.value.sdt,
  (v) => {
    if (laMoi) formDiaChi.value.sdt = v;
  },
);

// Tự động tạo tên đăng nhập cho khách hàng mới
watch(
  [() => form.value.email, () => form.value.sdt, () => form.value.hoTen],
  () => {
    if (!laMoi) return;
...
function validateDiaChi() {
  ...
  if (!f.phuongXa) {
    err.phuongXa = "Vui lòng chọn phường/xã.";
    ok = false;
  }
  if (!f.diaChiCuThe.trim()) {
    err.diaChiCuThe = "Vui lòng nhập địa chỉ cụ thể.";
    ok = false;
  }
  loiDiaChi.value = err;
  return ok;
  ...
  try {
    if (laMoi) {
      const result = await taoKhachHang({
        ...
        sdt: sdt || undefined,
        gioiTinh:
          form.value.gioiTinh !== "" ? Number(form.value.gioiTinh) : undefined,
    ...
    loiTrang.value = getDisplayErrorMessage(
      e,
      laMoi ? "Không thể tạo khách hàng" : "Không thể lưu thay đổi khách hàng",
    );
  } finally {
    dangLuu.value = false;
  }
...
async function luuDiaChi() {
  ...
  try {
    ...
    diaChiDangSua.value = null;
    showSuccess(
      dangSua ? "Đã cập nhật địa chỉ" : "Đã thêm địa chỉ mới",
      "Thành công",
    );
  } catch (e) {
...
async function xoaDiaChiKhachHang(diaChiId) {
...
async function datLamMacDinh(diaChiId) {
  try {
      ...
      <div class="flex flex-col-reverse xl:grid gap-5 xl:grid-cols-[320px_1fr]">
        ...
        <div class="space-y-4">
          ...
          <Card class="text-center">
              ...
              <img
                :src="
                  ...
                  'https://ui-avatars.com/api/?name=' +
                    encodeURIComponent(form.hoTen || khachHang?.hoTen || 'KH') +
            ...
            <p class="mt-4 text-base font-bold text-slate-800">
              {{ form.hoTen || khachHang?.hoTen || "Khách hàng mới" }}
            </p>
            <p class="text-sm text-slate-400">
              {{ form.email || khachHang?.email || "Chưa cập nhật email" }}
            </p>
        ...
        <div class="space-y-5">
            ...
            <div class="grid gap-4 sm:grid-cols-2">
              <label class="space-y-2">
                <span class="text-[13px] font-semibold text-slate-500"
                  >Họ và tên <span class="text-rose-500">*</span></span
                >
                <input
                  v-model="form.hoTen"
                  type="text"
              ...
              <label class="space-y-2 sm:col-span-2">
                ...
                />
                <p v-if="loiForm.ngaySinh" class="text-xs text-rose-500">
                  {{ loiForm.ngaySinh }}
                </p>
              </label>
            </div>
          ...
          <Card v-if="laMoi" class="space-y-5">
            <div class="flex items-center gap-3 mb-2">
              ...
              <div>
                ...
                <p class="text-sm text-slate-400">
                  Địa chỉ mặc định của khách hàng mới.
                </p>
              </div>
            </div>
          ...
          <Card v-else class="space-y-5">
            <div class="flex items-center justify-between gap-3">
              <div class="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p class="text-sm text-slate-400">
                    Quản lý các địa chỉ giao hàng của khách.
                  </p>
                </div>
              </div>
              <Button
            ...
            <div
              v-if="hienFormDiaChi"
              class="space-y-4 rounded-2xl border border-slate-100 bg-slate-50/60 p-5"
            >
              <div class="flex items-center justify-between">
                ...
                <Button
                  variant="ghost"
                  size="icon"
                  class="h-7 w-7 rounded-full bg-slate-100 text-slate-500 hover:bg-slate-200"
                  @click="huyFormDiaChi"
                >
                  <X class="h-4 w-4" />
                </Button>
            ...
            <div v-else>
              ...
              <div
                ...
                class="py-10 text-center border-2 border-dashed border-slate-100 rounded-3xl text-slate-400 text-sm"
              >
              ...
              <div v-else class="admin-table-scroll">
                <Table>
                  ...
                  <template #body>
                    ...
                    >
                      <td
                        class="rounded-l-2xl px-3 py-3 font-semibold text-slate-800 whitespace-nowrap"
                      >
                        {{ dc.hoTen }}
                      </td>
