...
const diaChiMacDinh = ref<any>(null);
const dsHoaDon = ref<any[]>([]);
...
function xemChiTietDon(hoaDonId: number) {
...
async function taiDuLieu() {
  ...
  try {
    ...
    khachHang.value = kh;
    diaChiMacDinh.value = dsDiaChi.find((dc: any) => dc.laMacDinh) ?? dsDiaChi[0] ?? null;
  } catch (e) {
...
<template>
  <div class="space-y-5">
    ...
    <div v-if="loiTrang" class="rounded-2xl bg-rose-50 border border-rose-100 px-5 py-3 text-sm font-medium text-rose-600">
      {{ loiTrang }}
    </div>

    <div v-if="dangTai" class="rounded-[24px] border bg-white p-10 text-center text-slate-400 text-sm">
      Đang tải...
    </div>
      ...
      <div class="flex flex-col xl:flex-row gap-5">
        ...
        <div class="xl:w-72 shrink-0">
          <Card class="text-center sticky top-5">
            <img
              :src="khachHang.hinhAnh || ('https://ui-avatars.com/api/?name=' + encodeURIComponent(khachHang.hoTen || 'KH') + '&background=f1f5f9&color=475569&size=128')"
              :alt="khachHang.hoTen"
              class="mx-auto h-24 w-24 rounded-full object-cover ring-4 ring-slate-100"
            />
            <p class="mt-4 text-base font-bold text-slate-800">{{ khachHang.hoTen }}</p>
            <p class="text-xs text-slate-400 mt-0.5">{{ khachHang.tenDangNhap }}</p>

            <span
              class="mt-2 inline-flex rounded-full px-3 py-1 text-[11px] font-semibold"
              :class="khachHang.trangThai === 1 ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'"
            >
              {{ khachHang.tenTrangThai }}
            </span>

            <div class="mt-4 space-y-2 text-left text-sm border-t border-slate-100 pt-4">
              <div v-if="khachHang.email" class="flex gap-2">
                <span class="w-16 shrink-0 text-[12px] font-semibold text-slate-400">Email</span>
                <span class="text-slate-700 break-all text-[12px]">{{ khachHang.email }}</span>
              </div>
              <div v-if="khachHang.sdt" class="flex gap-2">
                <span class="w-16 shrink-0 text-[12px] font-semibold text-slate-400">SĐT</span>
                <span class="text-slate-700 text-[12px]">{{ khachHang.sdt }}</span>
              </div>
              ...
              <div v-if="diaChiMacDinh" class="flex gap-2">
                ...
                <span class="text-slate-600 text-[12px] leading-relaxed">
                  {{ diaChiMacDinh.diaChiCuThe }}, {{ diaChiMacDinh.phuongXa }}, {{ diaChiMacDinh.quanHuyen }}, {{ diaChiMacDinh.tinhThanh }}
                </span>
              </div>
            </div>
            ...
            <div class="mt-4 border-t border-slate-100 pt-4">
              <Button
                ...
                @click="router.push({ name: 'admin-khach-hang-chi-tiet', params: { id } })"
              >
        ...
        <div class="flex-1 min-w-0">
          <Card no-padding>
            ...
            <template #title>
              Lịch sử đơn hàng
              <span v-if="dsHoaDon.length" class="ml-2 inline-flex items-center rounded-full bg-violet-100 px-2 py-0.5 text-xs font-bold text-violet-600">
                {{ dsHoaDon.length }}
              </span>
            </template>

            <div class="p-6">
              <div v-if="dangTaiDon" class="py-12 text-center text-sm text-slate-400">
                Đang tải đơn hàng...
              </div>
