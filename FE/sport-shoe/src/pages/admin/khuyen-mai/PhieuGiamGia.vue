...
const form = reactive({
  ...
  giamToiDa: "0", ngayBatDau: "", ngayKetThuc: "", soLuong: "", trangThai: "1"
});
...
async function openEditModal(target, item) {
  ...
  if (target === "phieu") {
    ...
    Object.assign(form, {
      ...
      giamToiDa: detail.giamToiDa ?? "0", ngayBatDau: detail.ngayBatDau ?? "",
      ngayKetThuc: detail.ngayKetThuc ?? "", soLuong: detail.soLuong ?? "",
    ...
    Object.assign(khForm, {
      ...
      email: "", // Tạm thời không gán lại vì FE không trả về email từ API detail cũ, người dùng tự nhập lại nếu muốn đổi
  ...
  try {
    if (modalTarget.value === "phieu") {
      const payload = {
        ...
        soLuongDaDung: 0, trangThai: Number(form.trangThai),
